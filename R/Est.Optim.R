#' Estimate item parameters and structural parameters via EM (internal optimizer)
#'
...
#'
#' @param dat Response data matrix of size \eqn{N \times J} (persons \eqn{N} by items \eqn{J}).
...
#' @param Q Q-matrix. Typically a binary matrix of size \eqn{J \times K} (items \eqn{J} by attributes \eqn{K}).
...
#' @param model Item model specification. A length-\eqn{J} vector (or scalar recycled) that is transformed
...
#' @param sequential Logical; whether to fit a sequential model (polytomous items recoded via \code{seq_coding}).
#' @param att.dist Character scalar or length-\eqn{G} vector specifying the attribute distribution model per group.
...
#' @param att.prior Prior for the joint attribute distribution. If \code{NULL}, initialized to uniform.
#'   If provided, should be a vector of length \eqn{2^K} or a matrix of size \eqn{2^K \times G},
#'   where \eqn{G} is number of groups.
#' @param saturated A list containing information for saturated attribute distribution updates.
...
#' @param att.str Logical; whether an attribute structure is imposed. When \code{TRUE}, only DINA/DINO/GDINA
#'   models are allowed and monotonicity constraints are disabled in this implementation.
#' @param mono.constraint Logical or logical vector of length \eqn{J}; whether to impose monotonicity constraints per item.
#' @param group Optional integer vector of length \eqn{N} indicating group membership (must start at 1).
#'   If \code{NULL}, a single group is assumed.
#' @param latent.var Logical; whether latent variance is modeled/used in initialization (NEED TO UPDATE).
#' @param verbose Integer; printing level. \code{0} silent, \code{1} progress in-place, \code{2} print each iteration.
#' @param catprob.parm Optional initial item parameters. If \code{NULL}, generated by \code{initials()}.
#'   If provided, converted by \code{l2m()} to a matrix (NEED TO UPDATE expected shape).
#' @param loglinear Integer scalar or length-\eqn{G} vector giving maximum interaction order used when
#'   \code{att.dist = "loglinear"}.
#' @param item.names Optional character vector of length \eqn{J} giving item labels. If \code{NULL}, default names are created.
#' @param solnp_args List of arguments passed to \code{solnp} (package \pkg{Rsolnp}) when used as optimizer (NEED TO UPDATE).
#' @param item.prior Optional item-parameter prior information passed to \code{MstepinOptim} (NEED TO UPDATE).
#' @param linkfunc Link function for delta parameterization. Can be \code{"identity"}, \code{"logit"}, or \code{"log"},
#'   either scalar or length-\eqn{J} vector; internally mapped to 1/2/3.
#' @param higher.order List of settings for higher-order model estimation when \code{att.dist = "higher.order"}.
#'   Contains quadrature nodes/weights and other options (NEED TO UPDATE full structure).
#' @param solver Character scalar or length-\eqn{J} vector specifying the optimizer per item (default \code{"auto"})
#'   (NEED TO UPDATE supported values).
#' @param auglag_args List of control arguments for augmented Lagrangian optimization (NEED TO UPDATE).
#' @param nloptr_args List of control arguments passed to \pkg{nloptr} (NEED TO UPDATE).
#' @param DesignMatrices Optional list of design matrices, length \eqn{J}. If \code{model} includes user-defined items,
#'   design matrices must be provided (NEED TO UPDATE exact requirements).
#' @param ConstrPairs Optional list (length \eqn{J}) defining partial orders used for monotonicity constraints
#'   (NEED TO UPDATE).
#' @param control A list of EM/optimization control parameters. Defaults include:
#'   \code{maxitr}, \code{conv.crit}, \code{conv.type}, \code{nstarts}, \code{lower.p}, \code{upper.p},
#'   \code{lower.prior}, and \code{randomseed}.
#'
#' @return A list containing estimated parameters and diagnostics, including:
#' \itemize{
...
#' }
#'
#' @seealso \code{\link{structural.parm}}
#' @keywords internal
Est.inOptim <- function(dat, Q, model, sequential,att.dist, att.prior,saturated,
...
# print(model)
 ...
 inputcheck(
  ...
  ConstrType <- rep(1,J)   # ConstrType =1: Constraint probability; ConstrType = 3: Constraint both probability & monotonicity
  ...
  while(itr < control$maxitr)
  {
    ...
    optims <- MstepinOptim(J=J, Kj = Kj, Rg = estep$Rg, Ng = estep$Ng, model = model, item.parm = item.parm, delta = delta,
